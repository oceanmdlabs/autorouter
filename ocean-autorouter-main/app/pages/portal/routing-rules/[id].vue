...
const { data: loadedData, status } = useAsyncData<RoutingRule | null>(`rule-${id}`, async () => {
	if (isNew) {
		return null;
	}
...
watch(() => loadedData.value, (newData) => {
	...
	if (!isNew && newData) {
		formValues.value = {
			...
			minutesSavedEstimate: newData.minutesSavedEstimate ?? null
		};
				...
				<div class="space-y-6">
					...
					<div class="space-y-2">
						<SelectGroup>
							...
							<Select v-model="formValues.triggeringEvent">
								<SelectTrigger>
					...
					<div class="space-y-2">
						...
						<Textarea id="prompt" v-model="formValues.prompt"
					...
					<!-- Minutes Saved Estimate Section -->
					<div class="space-y-2">
						<Label for="minutesSavedEstimate" class="flex items-center gap-2">
						...
						<p class="text-sm text-muted-foreground">
							Please provide an estimate of how many minutes of time would be saved if this rule executes successfully.
						</p>
						<div class="flex items-center gap-3">
							<Input
								id="minutesSavedEstimate"
								type="number"
								:model-value="formValues.minutesSavedEstimate ?? ''"
								@update:model-value="(val: string | number | null) => formValues.minutesSavedEstimate = val === '' || val === null ? null : Number(val)"
								min="1"
								max="100"
								placeholder="5"
								class="w-32"
							/>
							<span class="text-sm text-muted-foreground">minutes (optional, 1-100)</span>
							<Button
								v-if="formValues.minutesSavedEstimate !== null"
								type="button"
								variant="ghost"
								size="sm"
								@click="formValues.minutesSavedEstimate = null"
							>
								Clear
							</Button>
						</div>
						<p v-if="errors.minutesSavedEstimate" class="text-sm text-destructive">
							{{ errors.minutesSavedEstimate }}
						</p>
					</div>
				</div>
			</CardContent>
