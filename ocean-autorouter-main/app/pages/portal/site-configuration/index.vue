...
// Form values stores the site configuration without clinicSecret
// clinicSecret is stored in a separate ref and only used during new site creation
const formValues = ref<NewSiteConfiguration | SiteConfiguration | null>(null)
...
const showClinicSecret = ref(false);

// Clinic secret for new site encryption provisioning (not stored in formValues)
...
async function handleSubmit() {
  ...
  // Validate clinic secret for new configurations
  if (isNewConfig.value && (!clinicSecret.value || clinicSecret.value.length < 12)) {
    errors.value.clinicSecret = 'Clinic Secret is required and must be at least 12 characters'
    isSubmitting.value = false
    return
  }

  try {
    // Include clinicSecret for new configurations
    const body = isNewConfig.value
      ? { ...formValues.value, clinicSecret: clinicSecret.value }
      : formValues.value;
    ...
    if (isNewConfig.value) {
      // Clear the clinic secret after successful creation
      clinicSecret.value = '';
      toast.success('Site configuration created', {
        description: 'Zero-Knowledge encryption has been provisioned for your site.'
      });
    } else {
    ...
    <div class="space-y-6">
      ...
      <div v-else-if="showEmptyState">
        ...
        <Alert v-if="hadPreviousConfig" variant="warning" class="mt-10">
          ...
          you signed in with a different authentication provider.
        </Alert>
      </div>
      ...
      <form v-else-if="formValues" class="space-y-8" @submit.prevent="handleSubmit">
        ...
        <!-- Clinic Secret - Only shown for new configurations -->
        <Card v-if="isNewConfig" class="border-amber-200 bg-amber-50/50">
          <CardHeader>
            ...
            <CardDescription>
              Your Clinic Secret is used to encrypt sensitive data. This secret is never stored on our servers.
            </CardDescription>
          </CardHeader>
          <CardContent class="space-y-4">
            <Alert variant="warning" class="mb-4">
              <AlertTriangle class="h-4 w-4" />
              <AlertTitle>Important - Save This Secret</AlertTitle>
              <p class="text-sm mt-1">
                <strong>If you lose this secret, your encrypted data cannot be recovered.</strong>
                Store it securely (e.g., in a password manager). You will need it to view detailed audit information.
              </p>
            </Alert>
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <Input
                  ...
                  :type="showClinicSecret ? 'text' : 'password'"
                  placeholder="Enter a strong secret (min 12 characters)"
                  :aria-invalid="errors.clinicSecret ? 'true' : undefined"
                  class="pr-10"
                />
                <button
                  type="button"
                  class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700"
                  @click="showClinicSecret = !showClinicSecret"
                >
                  <Eye v-if="!showClinicSecret" class="h-4 w-4" />
                  <EyeOff v-else class="h-4 w-4" />
                </button>
              </div>
              <p class="text-xs text-gray-500">
                Use a strong, unique secret. Minimum 12 characters.
              </p>
              <p v-if="errors.clinicSecret" class="text-sm text-destructive">
                {{ errors.clinicSecret }}
              </p>
            </div>
          </CardContent>
              ...
              <div class="space-y-4">
                ...
                <div v-if="formValues" class="space-y-4">
                  ...
                  <Alert v-if="testConnectionResult"
                         ...
                         class="mt-2">
                    <AlertTitle>{{
                        testConnectionResult.success ? 'Successfully connected to Ocean'
                            :
                ...
                <div class="space-y-2">
                  <div v-if="lastSuccessfulConnection" :class="[
                    ...
                    <span class="text-sm">Last successful connection: {{
                        formatTimestampWithMinutePrecision(lastSuccessfulConnection)
                      }}</span>
                  </div>
                  <div v-else
              ...
              <div class="mt-6 space-y-4">
                ...
                <div class="space-y-4 bg-blue-50 p-4 rounded-lg">
                  ...
                  <Alert v-if="testSmsResult"
                    ...
                    <AlertTitle>{{
                        testSmsResult.success ? 'SMS sent successfully'
                            :
                            'SMS delivery failed'
                      }}
                    </AlertTitle>
            ...
            <CardContent>
              ...
              <div class="space-y-2">
                ...
                <Select id="aiProvider" v-model="formValues.aiProvider"
                  ...
                  <SelectContent>
                    <SelectItem value="vertex">Vertex (managed)</SelectItem>
              ...
              <!-- Vertex selected: show managed message -->
              <div v-if="formValues.aiProvider === 'vertex'" class="space-y-2 bg-gray-50 p-4 rounded-lg">
                <p class="text-sm text-gray-700">
                  The API connection configuration will be managed for you.
                </p>
              </div>
                ...
                <div class="space-y-2">
                  ...
                  <p class="text-sm text-gray-600">
                    Get your API key from the <a href="https://app.smtp2go.com/settings/api_keys"
                                                 target="_blank" class="text-blue-600 hover:underline">SMTP2GO
                    dashboard</a>
                  </p>
              ...
              <div class="mt-6 space-y-4">
                ...
                <div class="space-y-4 bg-blue-50 p-4 rounded-lg">
                  ...
                  <Alert v-if="testEmailResult"
                    ...
                    <AlertTitle>{{
                        testEmailResult.success ? 'Email sent successfully'
                            :
