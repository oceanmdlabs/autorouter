...
export async function processPatientEngagementEventUseCase(
  ...
  if (rules.length === 0) {
    ...
    details = siteConfig
      ? `Site Configuration ID: ${siteConfig.id}, name: ${siteConfig.name} has a patient engagement connection active but no Patient Engagement rules defined.`
      : `No Site Configuration found for tenant while processing Patient Engagement event.`;
