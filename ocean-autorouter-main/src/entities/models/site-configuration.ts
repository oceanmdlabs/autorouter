...
const AiProviderEnum = z.enum(["openai", "google", "cohere", "vertex"]);
...
const schema = baseResourceSchema.merge(tenantConfinedSchema).extend({
  ...
  emailFromAddress: z
    .union([
      z.string().email("Invalid email address"),  // Accept valid email
...
// Schema for new site configuration - requires clinicSecret for encryption provisioning
const newSchemaBase = schema.merge(newBaseResourceSchema);
const newSchema = newSchemaBase;

// Schema for new site creation request - includes clinicSecret for encryption provisioning
export const newSiteCreationRequestSchema = newSchemaBase.extend({
  clinicSecret: z.string()
    .min(12, "Clinic Secret must be at least 12 characters")
    .max(128, "Clinic Secret must be at most 128 characters")
});
export type NewSiteCreationRequest = z.infer<typeof newSiteCreationRequestSchema>;
