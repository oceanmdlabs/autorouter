...
export async function processMessageController(
  ...
  try {
    ...
    if (includeResult) {
      cxt.logger.info(`Successfully processed service request event type: ${eventContext.triggeringEvent}`,
        result
      );
    } else {
      cxt.logger.info(
        `Successfully processed service request event type: ${eventContext.triggeringEvent}`
      );
    }
...
async function loadAttachmentsIfIndicated(
  ...
  if (
    ...
    if (process.env.FETCH_ATTACHMENTS !== "false") {
      ...
      // skip fetching attachments in production due to runtime performance constraints
      cxt.logger.info(
