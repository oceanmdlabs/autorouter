...
export const createEvaluateRuleService = (deps: Dependencies) => {
  ...
  async function evaluateRule({
    ...
    cxt.logger.info(`Evaluating rule [${rule.name}] for request: [${requestDescription}]`);
    cxt.logger.debug(
    ...
    try {
      ...
      cxt.logger.debug(`Received tool calls for request ${requestDescription}`, {
      ...
      aiSuggestedActions.forEach(action => {
        ...
        if (process.env.LOG_LEVEL === "debug") {
          logObj.input = action.input;
        }

        cxt.logger.info("AI suggested action created", logObj);
      });
